...
AABB CollisionSystem::getAABB(const TransformComponent& transform,
    ...
    return AABB{transform.x + collision.offsetX,
                transform.y + collision.offsetY, collision.width,
                collision.height};
};

bool CollisionSystem::overlapSolidTile(const AABB& box,
...
glm::vec2 CollisionSystem::resolveMapCollision(float x, float y, float width,
                                               float height, float newX,
                                               float newY,
                                               const Tilemap& tilemap) {
