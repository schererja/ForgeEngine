...
void Camera::recalculateViewProjection() {
    // In this simple 2D camera, the view matrix is just a translation based on
    // the camera's position. Moving Camera to the right (positive x) should
    // move the world to the left, so we translate by -x.
